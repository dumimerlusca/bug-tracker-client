...
const TicketsProvider = ({ children }) => {
	...
	const getTickets = async query => {
		...
		try {
			const res = await axios.get(
				`${process.env.REACT_APP_BASE_URL}/api/v1/tickets${query ? query : ""}`
			);
	...
	const getMyTickets = async (userId, query) => {
		...
		try {
			const res = await axios.get(
				`${process.env.REACT_APP_BASE_URL}/api/v1/tickets?user=${userId}${
					query ? query : ""
				}`
			);
	...
	const getTicket = async id => {
		...
		try {
			const res = await axios.get(
				`${process.env.REACT_APP_BASE_URL}/api/v1/tickets/${id}`
			);
	...
	const addTicket = async (project, ticket) => {
		...
		try {
			await axios.post(
				`${process.env.REACT_APP_BASE_URL}/api/v1/projects/${project}/tickets`,
				ticket,
				config
			);
